...
@Component({
  ...
  `],
  template: `
    <select aria-label="seleziona mese" title="seleziona mese"
     [disabled]="disabled" class="custom-select d-inline-block" [value]="date?.month" (change)="changeMonth($event.target.value)">
      <option *ngFor="let m of months" [value]="m">{{ i18n.getMonthShortName(m) }}</option>
    </select>` +
      `<select aria-label="seleziona anno" title="seleziona anno"
       [disabled]="disabled" class="custom-select d-inline-block" [value]="date?.year" (change)="changeYear($event.target.value)">
